...
export function exportIncidenciasExcel(
  ...
  const rows = items.map((item) => {
    ...
    return {
      ...
      'Fecha dieta': item.dietaFecha ? formatDate(item.dietaFecha) : '',
      ...
      'Fecha trat.': item.tratamientoFecha ? formatDate(item.tratamientoFecha) : '',
      Proceso: formatListaConOtros(item.proceso, item.procesoOtros),
      'Fecha proceso': item.procesoFecha ? formatDate(item.procesoFecha) : '',
